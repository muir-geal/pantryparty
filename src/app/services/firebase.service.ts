...
import { addDoc, arrayUnion, collection, collectionData, deleteDoc, doc, documentId, FieldValue, Firestore, getCountFromServer, getDoc, getDocs, orderBy, query, updateDoc, where } from '@angular/fire/firestore';
  ...
  async addFoodToPantry(name:string = '')
  {
    ...
    */
    ...
    let food = [{
      ...
      "allergens":  // Achtung: Array sieht anders aus und muss erst umformattiert werden: "allergens": "en:eggs,en:milk,en:nuts,en:peanuts,en:soybeans",
      [
          "eggs",
          "milk",
          "nuts",
      ...
      "image": "https://images.openfoodfacts.net/images/products/590/095/131/1505/front_en.22.400.jpg", // image_url
      "nutrition": {
          "fat": 1,
          "saturated-fat": 2,
          "salt": 3,
      ...
      "nutriments": { // Direkt und komplett aus der Datenbank übernehmen!!!
        "carbohydrates": 60,
      ...
      "nutriments_estimated": { // Direkt und komplett aus der Datenbank übernehmen!!!
        "alcohol_100g": 0,
...
async deleteFoodFromPantry(foodItem: any) {
  ...
  try {
    ...
    const updatedFoods = currentFoods.filter((food: any, index: number) => index !== foodIndex);
...
async logEatenFood(food: EatenFood) {
  ...
  const eatenFoods = Array.isArray(pantryData?.['eatenFoods']) ? [...pantryData['eatenFoods']] : [];
